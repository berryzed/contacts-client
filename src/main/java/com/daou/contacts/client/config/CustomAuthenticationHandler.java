...
public class CustomAuthenticationHandler implements AuthenticationSuccessHandler, AuthenticationFailureHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest req, HttpServletResponse res, Authentication auth) throws IOException {
        ...
        ResponseEntity<Map<String, String>> response = restTemplate.exchange(apiUrl + "/token/{userId}", HttpMethod.POST, null, new ParameterizedTypeReference<Map<String, String>>() {
        }, loggedUser.getId());
        if (response.getStatusCode().isError() || response.getBody() == null) {
            throw new RuntimeException("토큰 정보 확인 불가");
        }

        loggedUser.setAccessToken(response.getBody().get("accessToken"));
